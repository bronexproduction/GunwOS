//
//  input.c
//  GunwOS
//
//  Created by Artur Danielewski on 08.12.2023.
//

#include "input.h"
#include "session.h"

#include <mem.h>
#include <proc.h>
#include <string.h>
#include <gunwdev.h>
#include <gunwipc.h>
#include <gunwinput.h>
#include <gunwfug.h>
#include <gunwlog.h>
#include <inputmgr.h>

extern sessionPtr_t inputStack[MAX_SESSION];
extern struct session sessions[MAX_SESSION];
extern void sessionClear(const sessionPtr_t sessionPtr);

static void onKeyboardEvent(const struct gnwDeviceEvent *const deviceEvent) {
    CHECKPTR(deviceEvent);
    CHECKPTR(deviceEvent->data);
    if (deviceEvent->dataSizeBytes != 1) {
        fug(FUG_INCONSISTENT);
        return;
    }

    struct gnwInputManagerKeyboardEventQuery query;
    query.keyboardEvent.code = deviceEvent->type;
    query.keyboardEvent.scancode = *(char *)(deviceEvent->data);

    if (query.keyboardEvent.code != GKEC_KEY_DOWN &&
        query.keyboardEvent.code != GKEC_KEY_UP) {
        fug(FUG_INCONSISTENT);
        return;
    }

    struct session * const session = sessionOnTop();
    if (!session) {
        return;
    }

    const enum gnwIpcError e = ipcSendDirect(session->procId, INPUTMGR_PATH_KEYBOARD_EVENT,
                                             (data_t){ (ptr_t)&query, sizeof(struct gnwInputManagerKeyboardEventQuery) },
                                             (data_t){ nullptr, 0 },
                                             (struct gnwIpcBindData){ GIBF_NONE, 0 });
    if (e == GIPCE_NONE) {
        return;
    }

    log("gnwinptm - onKeyboardEvent - ipcSendDirect failed - destroying session");
    sessionDestroy(session);
    
    if (e == GIPCE_FORBIDDEN || e == GIPCE_NOT_FOUND) {
        return;
    } else {
        char logMsg[75] = "gnwinptm - onKeyboardEvent - ipcSendDirect failure unexpected error        ";
        int2str(e, logMsg + 67);
        log(logMsg);
        fug(FUG_UNDEFINED);
    }
}

static void onMouseEvent(const struct gnwDeviceEvent *const deviceEvent) {
    CHECKPTR(deviceEvent);
    CHECKPTR(deviceEvent->data);
    if (deviceEvent->dataSizeBytes != sizeof(union gnwMouseEventData)) {
        fug(FUG_INCONSISTENT);
        return;
    }

    struct gnwInputManagerMouseEventQuery query;
    query.mouseEvent.code = deviceEvent->type;
    memcopy(deviceEvent->data, &(query.mouseEvent.data), sizeof(union gnwMouseEventData));

    if (query.mouseEvent.code != GMEC_KEY_DOWN &&
        query.mouseEvent.code != GMEC_KEY_UP &&
        query.mouseEvent.code != GMEC_MOVEMENT) {
        fug(FUG_INCONSISTENT);
        return;
    }

    struct session * const session = sessionOnTop();
    if (!session) {
        return;
    }

    const enum gnwIpcError e = ipcSendDirect(session->procId, INPUTMGR_PATH_MOUSE_EVENT,
                                             (data_t){ (ptr_t)&query, sizeof(struct gnwInputManagerMouseEventQuery) },
                                             (data_t){ nullptr, 0 },
                                             (struct gnwIpcBindData){ GIBF_NONE, 0 });
    if (e == GIPCE_NONE) {
        return;
    }

    log("gnwinptm - onMouseEvent - ipcSendDirect failed - destroying session");
    sessionDestroy(session);
    
    if (e == GIPCE_FORBIDDEN || e == GIPCE_NOT_FOUND) {
        return;
    } else {
        char logMsg[72] = "gnwinptm - onMouseEvent - ipcSendDirect failure unexpected error        ";
        int2str(e, logMsg + 64);
        log(logMsg);
        fug(FUG_UNDEFINED);
    }
}

static bool listen_keyboard() {
    struct gnwDeviceUHADesc keyboardDesc;
    enum gnwDeviceError e = devGetByType(DEV_TYPE_KEYBOARD, &keyboardDesc);

    if (e != GDE_NONE) {
        fug(FUG_UNDEFINED);
        return false;
    }

    e = devAcquire(keyboardDesc.identifier);
    if (e != GDE_NONE) {
        fug(FUG_UNDEFINED);
        return false;
    }

    e = devListen(keyboardDesc.identifier, onKeyboardEvent);
    if (e != GDE_NONE) {
        devRelease(keyboardDesc.identifier);
        fug(FUG_UNDEFINED);
        return false;
    }

    return true;
}

static bool listen_mouse() {
    struct gnwDeviceUHADesc mouseDesc;
    enum gnwDeviceError e = devGetByType(DEV_TYPE_MOUSE, &mouseDesc);

    if (e != GDE_NONE) {
        fug(FUG_UNDEFINED);
        return false;
    }

    e = devAcquire(mouseDesc.identifier);
    if (e != GDE_NONE) {
        fug(FUG_UNDEFINED);
        return false;
    }

    e = devListen(mouseDesc.identifier, onMouseEvent);
    if (e != GDE_NONE) {
        devRelease(mouseDesc.identifier);
        fug(FUG_UNDEFINED);
        return false;
    }

    return true;
}

bool input_init() {
    memzero(inputStack, sizeof(sessionPtr_t) * MAX_SESSION);
    for (size_t i = 0; i < MAX_SESSION; ++i) {
        sessionClear(&sessions[i]);
    }
    
    if (!listen_keyboard()) {
        return false;
    }
    if (!listen_mouse()) {
        return false;
    }
    
    return true;
}

enum gnwDeviceError input_attach(const procId_t procId) {
    struct session * inputSession = sessionForProc(procId);
    if (inputSession) {
        return GDE_ALREADY_HELD;
    }

    enum gnwDeviceError e = sessionCreate(procId, &inputSession);
    if (e != GDE_NONE) {
        return e;
    }
    if (!inputSession) {
        fug(FUG_NULLPTR);
        return GDE_UNKNOWN;
    }

    e = sessionEnable(inputSession);
    if (e != GDE_NONE) {
        sessionDestroy(inputSession);
        return e;
    }

    return GDE_NONE;
}
